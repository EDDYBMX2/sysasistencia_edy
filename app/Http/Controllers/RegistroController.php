    ...
    public function marcarsalida(Request $request){
        ...
        try {
            if ($_POST['latitud_entrada'] == ''){
                ...
                Image::make($image)->save(public_path("capturas_asistencia"."/".$filename));
                $registro->update();
                // Regsitramos las ventas en la salida
                $ventaSalida = new VentaSalida();
                $ventaSalida->fecVS = $mytime->toDateString();
                $ventaSalida->smart = $request->get('smart');
                $ventaSalida->kit = $request->get('kit');
                $ventaSalida->sim = $request->get('sim');
                $ventaSalida->plan = $request->get('plan');
                $ventaSalida->IdRegistro = $_POST['idregistro'];
                $ventaSalida->save();
            }
